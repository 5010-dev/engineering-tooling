...
func TestRunBuildsDeterministicReleaseManifest(t *testing.T) {
	...
	for _, name := range []string{
		"golden-path_1.2.0_darwin_amd64.tar.gz",
		"golden-path_1.2.0_darwin_arm64.tar.gz",
		"golden-path_1.2.0_linux_amd64.tar.gz",
		"golden-path_1.2.0_linux_arm64.tar.gz",
	} {
	...
	if result.ReleaseVersion != "1.2.0" || result.Lifecycle != "stable" || len(result.Enforcement) != 1 || result.Enforcement[0] != "report-only" || len(result.Assets) != 4 || len(result.RuntimeSelections) == 0 || result.Components.AssetBundle.Version != "1.2.0" || result.Components.AssetBundle.Digest == "" || result.Components.Checker.Digest == "" || result.Components.Generator.Digest == "" || result.Components.Automation.Version != "1.2.0" || result.Components.Automation.Digest == "" {
