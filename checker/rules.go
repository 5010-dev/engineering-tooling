...
func parseJustRecipes(value string) map[string]bool {
	return justsyntax.Recipes(value)
}
...
func parseJustImports(value string) []justImport {
	imports := justsyntax.Imports(value)
	result := make([]justImport, 0, len(imports))
	for _, imported := range imports {
		result = append(result, justImport{optional: imported.Optional, path: imported.Path})
	}
	return result
}
